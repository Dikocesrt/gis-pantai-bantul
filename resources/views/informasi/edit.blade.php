...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900">Edit Informasi</h1>
            <p class="text-gray-600 mt-2">Perbarui informasi atau berita</p>
        </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="2" required
                            ...
                            placeholder="Ringkasan singkat (1-2 kalimat)">{{ old('description', $informasi->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="content" name="content" rows="8" required
                            ...
                            placeholder="Tulis konten lengkap di sini. Tekan Enter untuk membuat paragraf baru.">{{ old('content', $informasi->content) }}</textarea>
                    ...
                    <div>
                        ...
                        @if ($informasi->image_url)
                            <img src="{{ $informasi->image_url }}" alt="{{ $informasi->title }}"
                                class="w-full max-w-md h-48 object-cover rounded-lg border-2 border-gray-200">
                        @else
                            <div class="w-full max-w-md h-48 bg-gray-200 rounded-lg flex items-center justify-center">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Format: JPEG, JPG, PNG, GIF, WEBP. Maksimal 10 MB. Kosongkan
                    ...
                    <div class="border-t border-gray-200 pt-6">
                        <div class="flex items-center gap-3 mb-4">
                            <input type="checkbox" id="is_event" name="is_event" value="1"
                                {{ old('is_event', $informasi->event_date ? '1' : '') ? 'checked' : '' }}
                        ...
                        <!-- Event Fields -->
                        <div id="event_fields"
                            class="space-y-6 {{ old('is_event', $informasi->event_date ? '1' : '') ? '' : 'hidden' }}">
                            <!-- Event Location -->
                            <div>
                                ...
                                <input type="text" id="event_location" name="event_location"
                                    value="{{ old('event_location', $informasi->event_location) }}"
                            ...
                            <div>
                                ...
                                <input type="date" id="event_date" name="event_date"
                                    value="{{ old('event_date', $informasi->event_date?->format('Y-m-d')) }}"
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="time" id="event_start_time" name="event_start_time"
                                        value="{{ old('event_start_time', $informasi->event_start_time) }}"
                                ...
                                <div>
                                    ...
                                    <input type="time" id="event_end_time" name="event_end_time"
                                        value="{{ old('event_end_time', $informasi->event_end_time) }}"
                                ...
                                <select id="tempat_wisata_id" name="tempat_wisata_id"
                                    ...
                                    @foreach ($tempatWisatas as $tempat)
                                        <option value="{{ $tempat->id }}"
                                            {{ old('tempat_wisata_id', $informasi->tempat_wisata_id) == $tempat->id ? 'selected' : '' }}>
                                            {{ $tempat->name }}
                                        </option>
                                    @endforeach
                                </select>
