...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div
            ...
            <div class="flex justify-between items-start">
                <div>
                    <p class="text-gray-600 text-sm font-medium mb-2">Total Informasi</p>
                    <h3 class="text-4xl font-bold text-gray-900">{{ $informasis->count() }}</h3>
                </div>
            ...
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-4xl font-bold text-gray-900">
                        {{ $informasis->where('created_at', '>=', now()->startOfDay())->count() }}</h3>
                </div>
            ...
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-lg font-bold text-gray-900">
                        {{ $informasis->first()?->updated_at?->diffForHumans() ?? 'Belum ada data' }}</h3>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="p-6 border-b border-gray-200 flex justify-between items-center">
            <div>
                <h3 class="text-xl font-bold text-gray-900">Daftar Informasi</h3>
                <p class="text-gray-600 text-sm mt-1">Kelola semua data informasi dan berita</p>
            </div>
            <a href="{{ route('informasi.create') }}"
                class="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-semibold rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($informasis as $index => $item)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="max-w-xs">
                                    <p class="text-sm font-semibold text-gray-900 line-clamp-2">{{ $item->title }}</p>
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">
                                {{ $item->published_at->format('d M Y, H:i') }}</td>
                            <td class="px-6 py-4">
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-gray-600 font-medium">Belum ada data informasi</p>
                                    <p class="text-gray-500 text-sm mt-1">Klik tombol "Tambah Informasi" untuk menambahkan
                                        data</p>
                                </div>
                            </td>
                        </tr>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full transform transition-all relative z-[101]">
            <div class="p-6 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <h3 class="text-2xl font-bold text-gray-900">Hapus Informasi</h3>
                ...
                <div class="mb-6">
                    <div class="flex items-center gap-4 p-4 bg-red-50 rounded-lg">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600 mt-1">Data informasi "<span id="delete_title"
    ...
    <script>
        function openDeleteModal(informasi) {
            document.getElementById('deleteModal').classList.remove('hidden');
            document.getElementById('delete_title').textContent = informasi.title;
            document.getElementById('deleteForm').action = `/admin/informasi/${informasi.id}`;
        }
