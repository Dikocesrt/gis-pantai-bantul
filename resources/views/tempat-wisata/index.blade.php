...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div
            ...
            <div class="flex justify-between items-start">
                <div>
                    <p class="text-gray-600 text-sm font-medium mb-2">Total Tempat Wisata</p>
                    <h3 class="text-4xl font-bold text-gray-900">{{ $tempatWisata->count() }}</h3>
                </div>
            ...
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-4xl font-bold text-gray-900">
                        {{ $tempatWisata->where('created_at', '>=', now()->startOfDay())->count() }}</h3>
                </div>
            ...
            <div class="flex justify-between items-start">
                <div>
                    ...
                    <h3 class="text-lg font-bold text-gray-900">
                        {{ $tempatWisata->first()?->updated_at?->diffForHumans() ?? 'Belum ada data' }}</h3>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
        <div class="p-6 border-b border-gray-200 flex justify-between items-center">
            <div>
                <h3 class="text-xl font-bold text-gray-900">Daftar Tempat Wisata</h3>
                <p class="text-gray-600 text-sm mt-1">Kelola semua data tempat wisata pantai</p>
            </div>
            ...
            <table class="w-full">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Gambar
                        </th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Nama
                            Tempat</th>
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tipe
                        </th>
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($tempatWisata as $index => $item)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div>
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">{{ Str::limit($item->address, 40) }}</p>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                @if ($item->rating_count > 0)
                                    <div class="inline-flex items-center gap-1.5 px-2.5 py-1.5 bg-yellow-50 rounded-lg">
                                        ...
                                        <span
                                            class="text-sm font-bold text-gray-900">{{ number_format($item->rating_average, 1) }}</span>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('tempat-wisata.edit', $item->id) }}"
                        ...
                        <tr>
                            <td colspan="9" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-gray-600 font-medium">Belum ada data tempat wisata</p>
                                    <p class="text-gray-500 text-sm mt-1">Klik tombol "Tambah Tempat Wisata" untuk
                                        menambahkan data</p>
                                </div>
                            </td>
                        </tr>
        ...
        <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full transform transition-all relative z-[101]">
            <div class="p-6 border-b border-gray-200">
                <div class="flex items-center justify-between">
                    <h3 class="text-2xl font-bold text-gray-900">Hapus Tempat Wisata</h3>
                ...
                <div class="mb-6">
                    <div class="flex items-center gap-4 p-4 bg-red-50 rounded-lg">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-600 mt-1">Tempat wisata "<span id="delete_name"
                                    class="font-semibold"></span>" akan dihapus permanen beserta semua gambar dan data
                                terkait.</p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function openDeleteModal(item) {
            ...
            document.getElementById('delete_name').textContent = item.name;
            document.getElementById('deleteForm').action = `/tempat-wisata/${item.id}`;
        }
